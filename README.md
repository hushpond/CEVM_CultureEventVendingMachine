...
System Overview
The project aims to solve the fragmentation of cultural information by providing a unified, persona-sensitive recommendation engine. It combines the factual grounding of RAG with the stylistic flexibility of a fine-tuned Large Language Model (LLM).
...
Instruction Tuning: Developed 130 high-quality instruction-input-output triplets.
...
Retriever: Implemented a metadata-based search module to extract relevant event context (Title, Place, Price, Category).
...
3. Training Process
Utilized SFTTrainer for supervised fine-tuning.

Monitored training loss convergence, achieving a reduction from an initial 1.8 to a final 0.7.

Performed hyperparameter sweeps across various LoRA ranks (8, 16, 32, 64) to find the optimal balance between computational overhead and model representational power.
...
VRAM Management: Mitigated GPU memory leaks during evaluation loops by implementing manual session restarts and runtime cache clearing.
...
The model successfully demonstrated persona-switching capabilities, such as prioritizing low-cost events for students and using appropriate honorifics for senior users.

Significantly reduced hallucinations by grounding responses in the retrieved metadata.
...
https://docs.google.com/spreadsheets/d/1xIgswlovpABGyzf0uKfSIBD9_TKWo2o_I3GVx2mHtI4/edit?usp=sharing
